...
public class RegisterForm extends Frame {
		...
		for (Component comp : mainPanel.getComponents()) {
			if (comp.isVisible() == true) {
				if(comp.getName().equals(card1Name)) {
					...
					return isValid(emailInput, email);
				} else if(comp.getName().equals(card2Name)) {
					return (isValid(nameInput, name) && isValid(passInput, pass) && isValid(repassInput, repass) 
							&& isEnough(passInput, repassInput, pass, repass) && isMatch(passInput, repassInput, pass, repass));
	...
	boolean isValid(TextField feild, Label label ) {
	...
	boolean isEnough(TextField feild1, TextField feild2, Label label1, Label label2 ) {
		if( feild1.getText().length() > maxLength && feild2.getText().length() > maxLength) {
			return true;
		} else {
			JOptionPane.showMessageDialog(this, label1.getText() + " va " + label2.getText() + " phai lon hon " + maxLength);
		}
		return false;
